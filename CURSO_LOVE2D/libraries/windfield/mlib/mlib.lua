...
local function getLineLineIntersection(...)
    ...
    elseif not slope1 then                         -- First is vertical
        x =
        x1                                         -- They have to meet at this x, since it is this line's only x
...
local function getClosestPoint(perpendicularX, perpendicularY, ...)
    ...
    elseif #input == 2 then -- Given perpendicularX, perpendicularY, slope, intercept
        ...
        x1, y1 = 1,
            slope and slope * 1 + intercept or
            1 -- Need x1 and y1 in case of vertical/horizontal lines.
    end
    ...
    elseif checkFuzzy(slope, 0) then -- Horizontal line
        ...
        local perpendicularIntercept = getYIntercept(perpendicularX, perpendicularY,
            perpendicularSlope)
...
local function isCircleCompletelyInsideCircle(circle1x, circle1y, circle1radius, circle2x, circle2y,
                                              circle2radius)
    if not checkCirclePoint(circle1x, circle1y, circle2x, circle2y, circle2radius) then return false end
    local Type = getCircleCircleIntersection(circle2x, circle2y, circle2radius, circle1x, circle1y,
        circle1radius)
...
local function getPolygonLineIntersection(x1, y1, x2, y2, ...)
    ...
    for i = 1, #input, 2 do
        local x1, y1, x2, y2 = getLineSegmentIntersection(input[i], input[i + 1],
            cycle(input, i + 2), cycle(input, i + 3), x3, y3, x4, y4)
...
local function getPolygonSegmentIntersection(x1, y1, x2, y2, ...)
    ...
    for i = 1, #input, 2 do
        local x1, y1, x2, y2 = getSegmentSegmentIntersection(input[i], input[i + 1],
            cycle(input, i + 2), cycle(input, i + 3), x1, y1, x2, y2)
        if x1 and not x2 then
            choices[#choices + 1] = { x1, y1 }
        elseif x2 then
            choices[#choices + 1] = { x1, y1, x2, y2 }
...
local function checkPolygonPoint(px, py, ...)
    ...
    for i = 1, #points, 2 do
        ...
        if points[i + 3] and checkFuzzy(points[i + 3], py) then
            points[i + 3] = py +
                .001 -- Do not need to worry about alternate case, since points[2] has already been done.
        end
...
local function getPolygonCircleIntersection(x, y, radius, ...)
    ...
    for i = 1, #input, 2 do
        local Type, x1, y1, x2, y2 = getCircleSegmentIntersection(x, y, radius, input[i],
            input[i + 1], cycle(input, i + 2), cycle(input, i + 3))
        if x2 then
...
local function isPolygonInsidePolygon(polygon1, polygon2)
    ...
    for i = 1, #polygon2, 2 do
        local result = false
        result = isSegmentInsidePolygon(polygon2[i], polygon2[i + 1], cycle(polygon2, i + 2),
            cycle(polygon2, i + 3), polygon1)
        if result then
            bool = true; break
        end
    end
...
local function isPolygonCompletelyInsideCircle(circleX, circleY, circleRadius, ...)
    ...
    local function isDistanceLess(px, py, x, y, circleRadius) -- Faster, does not use math.sqrt
        ...
        return distanceX * distanceX + distanceY * distanceY <
            circleRadius *
            circleRadius            -- Faster. For comparing distances only.
    end
